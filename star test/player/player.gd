...
@export var crouch_in_air : bool = false
@export var animated_sprite : AnimatedSprite2D
@export var crouch_animation_name : String
@export var default_animation_name : String
@onready var collistion = $collistion
var is_crouching:bool
#bool to see if player can't stand
var cant_stand : bool
...
func _physics_process(delta):
	...
	if can_crouch:
		cant_stand = $ShapeCast2D.is_colliding()
		if Input.is_action_pressed("crouch"):
			animated_sprite.play(crouch_animation_name)
			collistion.shape.size.y = 32
			if !is_crouching:
				$icon.position.y -= 8
			is_crouching = true

		if !Input.is_action_pressed("crouch") and !cant_stand:
				animated_sprite.play(default_animation_name)
				collistion.shape.size.y = 48
				if is_crouching:
					$icon.position.y += 8
				is_crouching = false
...
func ouch(x):
	print(x,pos.x)
	set_modulate(Color(1, 0, 0, 0.8))
	vel.y = jump_velocity# * .7
